...
- `Cmd + Shift + I` - новый чат
- `Cmd + N` - новый чат (если агент в фокусе)
- `Cmd + W` - закрыть чат (если агент в фокусе)
...
## Как запускать курсор-команду или скилл

1. Допустим, есть созданная команда в `.cursor/commands/check-styles.md`
2. В строке Агента ставим слэш `/имя-команды`
