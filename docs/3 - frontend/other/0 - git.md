...
7. Если все конфликты решены и перебазирование закончено, то можно форсированно пушить изменения дочерней ветки
   <CODE>git push origin -f</CODE>. Это команда опасна, так как форсированное отправление коммитов перезатирает неполученные коммиты из удалённого репозитория и переписывает их (удаляет)
