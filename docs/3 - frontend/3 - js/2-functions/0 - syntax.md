...
- Если у функции отсутствует `return`, то она возвращает `undefined`.
...
Если переменная - это `существительное`, то переменная - это `глагол` в **camelCase**, действие. Общее правило - в НАЗВАНИИ ФУНКЦИИ должен находиться глагол, описывающий то, что функция делает:
...
Для генераторых функций. При каждом новом вызове будет возвращаться новые значения. Используется только внутри `function*`.
