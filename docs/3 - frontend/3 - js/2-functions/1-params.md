...
Если параметры не передаются, то будут подставлены параметры по умолчанию. Аргументы с присвоением в функциях JS лучше размещать в конце, чтобы при вызове не писать undefined в начале, если параметров несколько
...
`args` и `rest` внутри тела функции являются массивами, поэтому над ними можно делать все операции с массивами.
