...
Более лаконичная запись цикла `for`. Не нужна переменная для итератора, условие и инкремент.
...
Вложенные циклы (Обычно используются для обработки вложенных массивов в текущем массиве)
