...
Растояние между элементами. Можно пользоваться, когда флекс-потомки не имеют фиксированной ширины и занимают всё пространство флекс-контейнера.

- Что `gap` — это шорткат для row-gap и column-gap, можно задавать два значения: gap: 8px 16px; (строка/столбец).
- Работает не только в `flex`, но и в `grid`; в `flex` корректно разделяет элементы даже при переносе строк (flex-wrap).
- `gap` добавляет промежуток только между элементами, а не по краям контейнера (в отличие от `padding/margin`).
- Можно напомнить о поддержке браузерами (в старых Safari были проблемы; сейчас в большинстве современных — ок).
- Единицы: px, rem, % — допустимы; % считается от inline-размера контейнера.
