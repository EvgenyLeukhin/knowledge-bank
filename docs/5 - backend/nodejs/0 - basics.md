...
### База JavaScript/TypeScript для backend
...
- структура проекта
- Backend на Express или Fastify
...
### Лучший стек для обучения я бы предложил такой:

`Node.js` + `TypeScript` +` Fastify/Express` + `PostgreSQL` + `Prisma` + `Docker`
