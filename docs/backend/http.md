...
**HyperText transfer protocol**. Протокол передачи данных между клиентом и сервером. Изначально был для передачи только html-файлов, а сейчас уже можно передавать всё что угодно (css, js, images, fonts, json и т.д.). Простой, быстрый, гибкий, многофункциональный.
...
Трехуровневая архитектура: Уровень представления --> Уровень логики --> Уровень данных (Клиент --> Серевер --> БД)
