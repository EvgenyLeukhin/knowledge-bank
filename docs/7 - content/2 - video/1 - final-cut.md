...
`Events` - больше как структурная директория, которая содержит импортированные материалы

`Project` - создаётся внутри `Event`

```
Library: Свадьба (большая библиотека для хранения)
  Event: 01 Исходники (хранение медиа-исходников)
    video_001.mov
    video_002.mov
  Event: 02 Монтаж (различные варианты монтажа)
    Project: Wedding Highlights
    Project: Full Film
