...
### IV вариант - через iCloud ([icloud.com](https://www.icloud.com))
...
1. Во встроенном редакторе приложения "Фото" на iOS. Если нужно быстро отредактировать "прямо на устройстве". HDR сохранеяется.
2. Во встроенном редакторе приложения "Фото" на Mac OS. (`Открыть` фото в медиатеке --> `Редактировать`). HDR сохранеяется.
3. В Adobe `Lightroom` как обычную raw-фотографию (проблемы с экспортом HDR, настройки сохраняются, а HDR нет). HDR не сохранеяется.
...
На iPhone обычно 2–3 камеры с разными фокусными расстояниями:

- **Ультраширокоугольная** (обычно ~13–14 мм эквивалент)
- **Основная/широкоугольная** (обычно ~24–26 мм эквивалент)
- **Телефото** (обычно ~52–77 мм эквивалент, если есть)
